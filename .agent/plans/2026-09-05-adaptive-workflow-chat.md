...
Phase 4 is accepted: 1001 default tests passed with nine separately passed Docker cases and one unperformed live-provider gate. `WorkflowEngine` implements durable direct/single/pair execution, exact Engineer/Verifier approval checkpoints, immutable patch verification and at-most-once tool dispatch. `FleetPlanner` currently rejects the two advanced strategies. `FleetPlanNode` has scopes/dependencies but lacks subgoals and criterion assignments. `PydanticAIRuntimeAdapter._invoke_model` resets invocation accounting/history and reports only successful usage. SQLite schema 4 has no graph/conversation/budget tables. `EvidenceAssembler` intentionally makes multi-criterion claims inconclusive. `GitRepositoryAdapter.compute_patch` currently omits untracked files.
...
- [ ] Freeze, implement and accept advanced adaptive graph slice.
...
Milestone 1 is accepted on the frozen pre-graph working tree on 2026-09-05. Simple direct/single/pair workflows now retain cumulative runtime accounting across pauses/failures/cancellation, honor role/delegation ceilings, provide bounded repair feedback and substantive direct responses, and deliver text-only new-file patches with exact criterion mappings. Legacy runs without reliable accounting remain `legacy_unknown` and cannot acquire an invented zero budget. Phase 4 (`2e93092`) remains the last accepted whole phase; this working-tree acceptance is not a new commit, merge or release claim.
