...
本文只描述这个仓库的 Agent Fleet 产品，不把开发它所用的 Codex 工具、插件、记忆或 GitHub 操作能力算作产品能力。历史规格中存在 target、旧阶段状态和概念性目录树；当前实现以实际注册的适配器、调用路径和最新验收记录为依据。
