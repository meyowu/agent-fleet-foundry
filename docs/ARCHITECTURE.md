...
Implementation boundary: Phase 0–5 is accepted: durable state/artifacts, hardened Git worktrees, bounded repository intelligence, guarded patch application, exact permissions and external user trust, cumulative budgets, all five adaptive strategies and persistent bounded chat (sections 13–14). Fake and BYOK PydanticAI runtimes share project-owned contracts and gateway-backed tools; the live adapter supports only wired `openai:` and `openai-chat:` families, but live-provider acceptance was not run. Docker is the sole current isolated provider; fake is simulated and local-unsafe non-isolating. Exact historical and current gates are in `MVP_ACCEPTANCE.md`. Operational FleetPatch (Phase 6) and release hardening/guide (Phase 7) remain unimplemented. Remote sandboxes are post-MVP; complete-MVP, fresh-user/platform, final GitHub and live-provider/license gates remain open.
