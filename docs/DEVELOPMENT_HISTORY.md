# Development history and acceptance records

This is the previous homepage, preserved on 2026-09-12 under its original
Agent Fleet name. Some entries describe unmerged development in the source
worktree when the snapshot was captured; they do not claim that code ships in
this documentation delivery. Repository navigation links use the new name. Statements, examples,
license status and verification counts below describe their dated snapshots;
they are not current setup instructions. Use the [public quickstart](../README.md)
and [user guide](USER_GUIDE.md) for onboarding. New progress belongs in the
[continuation plan](../.agent/plans/2026-09-11-p0-p1-completion.md) and
[acceptance ledger](MVP_ACCEPTANCE.md).
...
## P0/P1 continuation — 2026-09-11–12
...
check passed1/8.69s. [PR8](https://github.com/meyowu/agent-fleet-foundry/pull/8)
merged at2026-09-12 06:36:48UTC as `3bcf735329d4af312c3fe58f408b3f10f901b29e`;
its tree equals the accepted head and local/remote main were synchronized.
Actions readback returned zero runs for both head and merge SHAs (`[skip ci]`),
...
Slice B adds explicit live Canary role selections (unmerged working-tree development).
The repaired offline candidate passed independent verification:116 focused
tests,34 unsafe credential-destination rejections, prompt FIFO/symlink/size
rejection, and six public-profile/SQLite binding readbacks. Its first exhaustive
matrix finished **4174 passed,23 skipped,1 failed** across4198 unique identities,
with no gaps/duplicates and548 unchanged source inputs. The failure was an old
release-prerequisite unit fixture still supplying the removed direct-pytest
environment interface. All integration groups (249/264/251), the separate
cancellation case and six static gates passed; the failed matrix is retained.
Only that synthetic fixture was corrected: ten fresh cases now cover strict
selection JSON, both opt-ins, the image and all three selected keys, plus
malformed/legacy-only rejection. No gate, provider code, limit or permission
changed. Independent review confirmed that the other547 source/test inputs,
including all seven previously reviewed B files, are byte-identical. Fresh
collection contained4201 identities. Explicit source-closure reconciliation
passed **4178 cases with23 skips**:4167 earlier passing identities and23 skips,
ten freshly re-executed prerequisite cases and one fresh distribution case
(1 passed/4.19s). The complete old prerequisite module and distribution case
were excluded from reuse. Carried-forward tests are not claimed as re-executed.
Neither these offline results nor an admitted selection is a new Provider/Harness
live qualification result.

The first mixed-Harness attempt on September12 was **NOT_PASSED** (one failed
test/8.94s; launcher10.523s). It stopped at role-binding admission: the initialized
catalog contains five roles, but the fixture configured only three and no default.
Independent ledger inspection found **zero target model dispatches**; only the
separate fake bootstrap ran. This does not establish key validity or a model
failure. Cleanup completed; the original failed attempt is retained. The fixture
repair must explicitly cover ancillary roles and pass a real Workflow-admission
regression without weakening production missing-binding guards. The preceding
source-closure acceptance is historical, not acceptance of that new repair.

## S1–S3 development status — 2026-09-10 UTC

**Merged to main:** [PR7](https://github.com/meyowu/agent-fleet-foundry/pull/7)
...
force at that snapshot and was later lifted only for the scoped continuation
above. Final documentation/package and remote
...
are separate commands. See the [Chinese walkthrough](USER_GUIDE.md#不调用模型的项目基线检查standalone-cli).
...
[contracts](CONFIG_AND_SCHEMAS.md#evaluation-contracts-s1-foundation-no-execution-authority)
...
Custom `.fleet/agents/roles.yaml` templates inherit only an Engineer, Verifier, Researcher or Architect execution kind. Reviewed FleetPatch changes version the catalog and guidance. Tools, paths and steps can be narrowed, not expanded beyond the base role; names remain exact permission principals. Custom parallel plans must explicitly select a compatible full-parent repair Engineer. See the [Chinese guide](USER_GUIDE.md) and [configuration contract](CONFIG_AND_SCHEMAS.md#12-session-first-model-and-role-contracts).

The [detailed user guide (简体中文)](USER_GUIDE.md), also bundled in the distribution, covers installation, initialization, BYOK, chat, adaptive teams, exact permissions, evidence/code review, organization evolution and both recovery procedures. [PR #5](https://github.com/meyowu/agent-fleet-foundry/pull/5), the [completion plan](../.agent/plans/2026-09-05-mvp-completion.md) and the [acceptance ledger](MVP_ACCEPTANCE.md) record the accepted local candidate and GitHub delivery separately from public release.
...
For a first exercise, use the [packaged learning project and step-by-step guide](USER_GUIDE.md#公开学习项目无需模型-key). Copy into a new directory, create its initial Git commit, select a disjoint state directory, and use fake runtime plus real Docker. Review exact approvals, inspect `verified_complete` and command evidence, then apply the code explicitly. This is genuine isolated testing with a deterministic model, not live model reasoning.
...
The local Docker image `agent-fleet-runner:0.1.0-py314-v1` must already exist; `--image` may select another prepared compatible local image. By default the launcher pins PydanticAI and `openai:gpt-5-nano` for every role. Optional `--selection FILE` uses a strict reviewed per-role selection (unmerged at this snapshot); dedicated `FLEET_` credential references, supported combinations and regular-file admission are checked before credential lookup or dispatch. The launcher strips unrelated provider/debug/pytest settings, never pulls or installs, and refuses a reused output directory. It sets live/Docker opt-ins for its child only. Missing selected credentials fail before launching; ordinary pytest still skips the live case. Selection digests bind the attempt and evidence; admitted combinations are not automatically live-qualified.
...
| Linux/macOS × Python3.12–3.14 | Final PR head `e413192` passed **all nine jobs** in [workflow33987810067](https://github.com/meyowu/agent-fleet-foundry/actions/runs/33987810067): each macOS selection1664 passed; each Linux selection1659 passed/five Darwin skips. Complete Linux default1997 passed/25 skips1031.53s; standalone security729 passed/five Darwin skips142.20s; installed2 passed/one deselected56.55s; Docker plus installed public journey15 passed/six deselected181.22s. |
