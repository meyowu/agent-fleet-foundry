...
This document distinguishes implemented contracts from targets. Phase 0–3 provides repository intelligence, exact configuration/task/evidence bindings, bounded fake/Docker/local-unsafe execution, bootstrap canaries and shared fake/PydanticAI runtime contracts. Phase 4's current-policy intersection, user-owned trust store, exact once/run/project approvals and permission CLI were accepted on 2026-09-05: `1001 passed, 10 skipped` in the default suite, with nine separately passing real-Docker tests. See `MVP_ACCEPTANCE.md` for the exact snapshot. No live-provider acceptance was run. Persistent chat (the remaining Phase 5 slice), operational FleetPatch (Phase 6), release hardening (Phase 7) and remaining live-provider/license gates stay open.

Phase 5 Milestone 1 (`ab28aaa`) additionally accepted cumulative budgets and typed criterion mapping. Milestone 2's graph execution is accepted on the development branch: 1344 default tests passed, with twelve Docker cases separately passing. The current catalog contains 55 generated schemas and SQLite migrations through `0006`; persistent chat is not yet implemented and whole-phase metadata remains 4.
