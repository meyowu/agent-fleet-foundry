...
The initial audit on 2026-09-05 inspected the Phase 3 baseline at commit `c700de1fc357844113426d3352cf29b6ffeae0f1`, tree `eb778abcc5c073d4c58f54e6a55c7a04d6333ea4`. **Phase 4 and Phase 5 M1/M2 are accepted by E4/E5.1/E5.2 below.** Phase 4/M1 are committed and pushed through `ab28aaa` on the development branch; M2's checkpoint is pending. Main remains the initial baseline and whole-phase metadata remains 4 until chat acceptance. No complete MVP release is claimed. Exact test results are retained in the ExecPlans; a separate durable raw log path was not recorded. Older entries remain historical unless explicitly superseded.
...
- **Accepted slices (E5.1/E5.2):** durable budgets/evidence and adaptive graph execution passed their named gates, including documentation-final archive read-back; persistent chat and whole Phase 5 remain open. M2 checkpoint remains pending.
...
All rows concern the roadmap's three-state permission lifecycle. Grants never expand a protected or effective task ceiling. **Accepted on 2026-09-05 under E4.** The observations below remain regression obligations; E5.1/E5.2 preserve them through budgets and graph execution. Chat, full Phase 5 and Phase 6/7 remain open.
...
| F5-09 | Restart/resume and approval pause. | E4/E5.1/E5.2 accept exact role/child approval restoration, durable budgets, exclusive driver/continuation ownership and stopped-owner recovery. Chat context remains missing; arbitrary provider history is not restored. | Complete conversation restart; uncertain dispatched work is abandoned/recovered, never silently replayed or treated as free. P5. |
| F5-10 | Budget enforcement and usage summary. | Accepted E5.1/E5.2 for immutable run/child shared limits, logical role caps, usage/unknown accounting, concurrent reservations and active-time watcher. Conversation selection remains future. | Preserve cumulative limits across chat-linked runs at the specified boundary; token usage is post-response accounting, not guaranteed pre-spend cost. P5. |
...
| --- | --- | --- |
| R1 Default suite passes offline after dependencies are installed. | E5.2: 1344 passed, 13 explicitly gated skips on the unchanged source/test checksum. | Repeat Q1–Q5 on the eventual full-MVP release tree; current success does not pre-accept later code. |
...
| U8 | Restart CLI and inspect/resume. | E4/E5.1/E5.2 accept exact role/graph approval restoration, durable accounting, exclusive dispatch and conservative stopped-owner recovery; chat remains missing. | Complete P5/P6 conversation and configuration-operation state; do not infer generic replay safety or SDK history restoration from approval checkpoints. |
...
Commands below identify the proving surface. P4 and the P5 execution selection name present tests; **the separate P5 chat and P6–P7 proposed files remain future requirements, not runnable evidence today.** Names may be updated with equivalent explicit case mapping. No command has been run merely by being listed.
...
P5 — future persistent-chat/restart targets (not implemented or run by E5.2):
...
| 2026-09-05 / E5.2 accepted, checkpoint pending | Frozen graph full/static/Docker/E2E/package gates and independent delivery audit; unchanged source/test checksum recorded above. | `1344 passed, 13 skipped`; Docker `12 passed`, zero managed resources; E2E `9 passed`; independent audit `11 passed`; final archive/README tests `4 passed in 4.37s`, repeat `4 passed in 1.73s`. Retained pre-final `2 failed, 212 passed, 85 deselected` superseded by fixes and final gates. | Graph-specific F5-03/F5-04/F5-09/F5-10/F5-11; not chat, whole Phase 5, Phase 6/7 or external gates. |
