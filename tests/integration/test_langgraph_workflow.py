"""Real StateGraph and Fleet persistence/Gateway, with explicitly simulated sandbox evidence."""
...
_KEY = "offline-langgraph-workflow-credential"
_MODEL = "gpt-test"
...
async def test_real_langgraph_workflow_respects_all_broker_decisions_and_retains_evidence(
    tmp_path: Path,
    ...
    def respond(received: httpx2.Request) -> httpx2.Response:
        ...
        assert received.headers["authorization"] == f"Bearer {_KEY}"
        assert body["model"] == _MODEL and _KEY not in received.content.decode()
        terminal = next(
            tool["name"] for tool in body["tools"] if tool["name"].startswith("submit_")
        )
        ...
        if terminal == "submit_scope_decision":
            ...
            # This is a fresh Fleet invocation after explicit approval, not a restored graph state.
            calls = [command]
        ...
        return httpx2.Response(
            ...
            json={
                ...
                "output": [
                    {
                        "type": "function_call",
                        "id": f"item-{number}-{index}",
                        "call_id": f"call-{number}-{index}",
                        "name": name,
    ...
    assert run.runtime_name == "langgraph" and run.provider_model == f"openai:{_MODEL}"
    ...
    for item in usages:
        ...
        assert data["provider_metadata"]["provider"] == "openai"
        assert data["provider_metadata"]["model"] == f"openai:{_MODEL}"
